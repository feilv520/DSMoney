...
@implementation MCMPieChartView
...
//        [self sortValueArr:valueArr colorArr:colorArr];
        self.rotatedView = [[MCMRotatedView alloc]initWithFrame:self.bounds];
        ...
        self.title.textColor = [self colorFromHexRGB:@"5f6d7b"];
        self.title.text = @"";
        [self.centerView addSubview:self.title];
        
        self.amountLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, (centerImage.size.width - 22) * 0.44, self.centerView.frame.size.width , 22)];
        self.amountLabel.backgroundColor = [UIColor clearColor];
        self.amountLabel.textAlignment = NSTextAlignmentCenter;
        self.amountLabel.font = [UIFont boldSystemFontOfSize:21];
        self.amountLabel.textColor = [self colorFromHexRGB:@"ff7624"];
...
- (void)selectedFinish:(MCMRotatedView *)rotatedView index:(NSInteger)index percent:(float)per
{
